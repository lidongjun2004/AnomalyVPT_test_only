# --------------------------------------------------------
# Adapted from  https://github.com/microsoft/unilm/tree/master/beitnn
# --------------------------------------------------------
import math
import os
...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        ...
        if self.rope:
            grid_count = 576
            # slightly fast impl
            q_t = q[:, :, 1:grid_count + 1, :] # [B, n_head, grid_count + prompt_length, C]
            q_vp = q[:, :, grid_count + 1:, :]
            ro_q_t = self.rope(q_t) # [B, n_head, grid_count, C]
            q = torch.cat((q[:, :, :1, :], ro_q_t, q_vp), -2).type_as(v)

            k_t = k[:, :, 1:grid_count + 1, :]
            k_vp = k[:, :, grid_count + 1: ,:]
            ro_k_t = self.rope(k_t)
            k = torch.cat((k[:, :, :1, :], ro_k_t, k_vp), -2).type_as(v)
...
class LearnablePrompt(nn.Module):
    ...
    def forward(self, x, grid_size=(24, 24), idx=0):
        cat_prompt = self.p[idx].unsqueeze(0).repeat(x.shape[0], 1, 1)
        x = torch.cat([x[:, : (grid_size[0] * grid_size[1] + 1), :], cat_prompt], dim=1)
...
class EVAVisionPromptTransformer(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        if prompt_layer_idx is not None and prompt_layer_idx.__len__() > 0:
            self.learnable_prompt = LearnablePrompt(
                len(prompt_layer_idx), prompt_length, embed_dim
            )
            ...
            if self.grad_checkpointing:
                ...
                if self.prompt_layer_idx is not None and i in self.prompt_layer_idx:
                    x = self.learnable_prompt(
                        x, grid_size=self.grid_size, idx=self.prompt_layer_idx.index(i)
                    )
                    x = blk(x, rel_pos_bias=rel_pos_bias)
                else:
                    x = blk(x, rel_pos_bias=rel_pos_bias)
    ...
    def forward(self, x, return_all_features=True):
        ...
        x = self.norm(x)
        res['pooled'] = self.head(x[:, 0])
        res['tokens'] = self.head(x[:, 1:])
        return res
