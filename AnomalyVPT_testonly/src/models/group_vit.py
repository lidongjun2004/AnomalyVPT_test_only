...
class GroupingBlock(nn.Module):
    ...
    Args:
        ...
        gumbel_tau (float): Temperature for gumbel softmax. Default: 1
    """
...
class GroupingPrompt(nn.Module):
    def __init__(self, cnt=1, length=[16], width=1024):
        assert cnt == len(length)
        super(GroupingPrompt, self).__init__()
        self.p = nn.ParameterList()
        for idx, l in enumerate(length):
            prompt_shape = (l, width)
            self.p.append(nn.Parameter(torch.zeros(prompt_shape)))

    def forward(self, x, idx=0):
        cat_prompt = (self.p[idx].unsqueeze(0).permute(1, 0, 2).repeat(1, x.shape[1], 1))
        # x = torch.cat([x[:(grid_size[0] * grid_size[1] + 1), :, :], cat_prompt], dim=0)
        x = torch.cat([cat_prompt, x], dim=0)
        return x


class GroupingBlockSet(nn.Module):
    def __init__(self,
                 dim=768,
                 out_dim=768,
                 num_heads=[8, 8, 8, 8],
                 num_group_tokens=[16, 8, 4, 2],
                 num_output_groups=[16, 4, 2]):
        super(GroupingBlockSet, self).__init__()
        assert num_group_tokens.__len__() == num_heads.__len__()
        norm_layer = nn.LayerNorm
        self.downsamples = nn.ModuleList()
        for idx, l in enumerate(num_output_groups):
            group_block = GroupingBlock(
                dim=dim,
                out_dim=out_dim,
                num_heads=num_heads[idx],
                num_group_token=num_group_tokens[idx],
                num_output_group=num_output_groups[idx],
                norm_layer=norm_layer,
                hard=True,
                gumbel=True)
            self.downsamples.append(group_block)

    def forward(self, x, group_token, idx=0, return_attn=True):
        # x: [32, 576, 1024] = [B, L, C]
        # group_token: [32, 16/4/2, 1024] = [B, S_1, C]
        x, attn_dict = self.downsamples[idx](x, group_token, return_attn=return_attn)
...
class GroupViT(nn.Module):
    ...
    def forward(self, x, *, return_feat=False, return_attn=False, as_dict=False):
        ...
        outs['x'] = self.forward_image_head(x)

        if return_feat:
            outs['feat'] = x_feat

        if return_attn:
            outs['attn_dicts'] = attn_dicts
