...
def thresholds_between_0_and_1(num_thresholds: int = 100, device: Optional[torch.device] = None) -> torch.Tensor:
...
class AUPRO(Metric):
    ...
    num_thresholds: Optional[int]

    def __init__(
        self,
        dist_sync_on_step: bool = False,
        process_group: Optional[Any] = None,  # noqa: ANN401
    ...
    def perform_cca(self) -> torch.Tensor:
        ...
        return connected_components_cpu(target)
    ...
    def compute_pro(
        ...
        if self.num_thresholds is not None:
            ...
            if torch.all((preds >= 0) * (preds <= 1)):
                ...
                thresholds = thresholds_between_0_and_1(
                    self.num_thresholds, self.device)
        ...
        new_idx = torch.arange(
            0, output_size, device=preds.device, dtype=torch.float)
        ...
        for label in labels:
            ...
            if not torch.allclose(_fpr[_fpr_idx].max(), self.fpr_limit):
                ...
                _slope = 1 - ((_fpr[_tmp_idx] - self.fpr_limit) /
            ...
            if interp:
                ...
                new_idx[-1] = _fpr_idx[-2] + \
                    ((_fpr_idx[-1] - _fpr_idx[-2]) * _slope)
